...
export default function PageFAQ({ pageFaqData }) {
  ...
  return (
    <div className="mx-auto p-6 md:flex w-full max-w-[1277px]">
      <h2 className="text-2xl text-center mb-6 font-medium text-[28px] w-[87%] mx-auto md:w-[30%] md:p-[22px] md:text-left">Your Questions, Answered</h2>
      <div>
        {/* Desktop Tabs */}
        {!isMobile && (
          <div className="flex flex-wrap gap-4 justify-center mb-6 border-b pb-6 border-[#6C6F73]">
            {faqData.map((tab, idx) => (
              <button
                key={idx}
                className={`px-4 py-2 rounded-md transition ${
                  activeTab === idx
                    ? 'text-blue-600 font-semibold relative md:after:absolute md:after:bottom-[-26px] md:after:border-b-[2.4px] md:after:border-[#0085FF] custom-after-empty  md:after:block md:after:w-full md:after:left-0'
                    : 'text-gray-700'
                }`}
                  ...
                  <div className="space-y-4 px-[15px] py-0">
                    {tab.qa.map((item, qIdx) => (
                      ...
                      >
                        ...
                        {openQuestions[tabIdx] === qIdx && (
                          <p className="text-gray-600 mt-2 md:text-[18px]">{item.answer}</p>
                        )}
                      </div>
                    ))}
                  </div>
